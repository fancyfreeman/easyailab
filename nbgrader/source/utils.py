...
# add by chenxin 
def get_username():
    """Get the username from environment variable NBG_USER or of the current process if NBG_USER is not set."""
    if pwd is None:
        raise OSError("get_username cannot be called on Windows")

    username = os.environ.get('JUPYTERHUB_USER')
    logging.warning("chenxin, %s",username)
    if username:
        return username
    return pwd.getpwuid(os.getuid())[0]
...
def self_owned(path):
    """Is the path owned by the current user of this process?"""
    return pwd.getpwuid(os.getuid())[0] == find_owner(os.path.abspath(path))
